-- =====================================================================================
-- COMPLETE SHARING SYSTEM REMOVAL (CLEAN VERSION)
-- =====================================================================================
...
BEGIN
    ...
    DROP POLICY IF EXISTS "Users can delete own share access records" ON share_access;
...
-- =====================================================================================
-- STEP 3: RESTORE ORIGINAL RESOURCE POLICIES (before sharing modifications)
-- =====================================================================================

DO $$
BEGIN
    -- Remove modified resource policies first
    DROP POLICY IF EXISTS "Users can view own resources and shared resources" ON resources;
    DROP POLICY IF EXISTS "Users can update own resources and shared resources with edit access" ON resources;
    DROP POLICY IF EXISTS "Users can delete own resources only" ON resources;

    -- Remove modified resource_type_config policies
    DROP POLICY IF EXISTS "Users can view resource type config and shared configs" ON resource_type_config;
    DROP POLICY IF EXISTS "Users can update resource type config and shared configs with edit access" ON resource_type_config;
    ...
    CREATE POLICY "Users can view their own resource type config"
        ON resource_type_config FOR SELECT
        TO authenticated
        USING (auth.uid() = user_id);

    CREATE POLICY "Users can create resource type config"
        ON resource_type_config FOR INSERT
        TO authenticated
        WITH CHECK (auth.uid() = user_id);

    CREATE POLICY "Users can update their own resource type config"
        ON resource_type_config FOR UPDATE
        TO authenticated
        USING (auth.uid() = user_id)
        WITH CHECK (auth.uid() = user_id);

    CREATE POLICY "Users can delete their own resource type config"
        ON resource_type_config FOR DELETE
        TO authenticated
        USING (auth.uid() = user_id);
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error restoring original policies: %', SQLERRM;
END $$;

-- =====================================================================================
-- STEP 4: DROP FUNCTIONS (before dropping dependencies)
-- =====================================================================================
...
BEGIN
    ...
    DROP FUNCTION IF EXISTS get_user_sharing_permission(UUID, UUID) CASCADE;
    DROP FUNCTION IF EXISTS can_user_access_resource(UUID, UUID) CASCADE;
    DROP FUNCTION IF EXISTS handle_share_invitation_acceptance(UUID, UUID, UUID) CASCADE;
    DROP FUNCTION IF EXISTS cleanup_expired_invitations() CASCADE;
    DROP FUNCTION IF EXISTS get_resource_sharing_info(UUID) CASCADE;
    DROP FUNCTION IF EXISTS validate_sharing_permission(TEXT) CASCADE;
...
-- =====================================================================================
-- STEP 5: DROP INDEXES (before dropping tables)
-- =====================================================================================

DO $$
BEGIN
    -- Drop sharing-related indexes
    DROP INDEX IF EXISTS idx_resource_shares_resource_id CASCADE;
    DROP INDEX IF EXISTS idx_resource_shares_user_id CASCADE;
    DROP INDEX IF EXISTS idx_resource_shares_token CASCADE;
    DROP INDEX IF EXISTS idx_share_invitations_share_id CASCADE;
    DROP INDEX IF EXISTS idx_share_invitations_token CASCADE;
    DROP INDEX IF EXISTS idx_share_invitations_email CASCADE;
    DROP INDEX IF EXISTS idx_share_access_share_id CASCADE;
    DROP INDEX IF EXISTS idx_share_access_user_id CASCADE;
...
BEGIN
    ...
    IF sharing_tables = 0 AND sharing_functions = 0 AND sharing_types = 0 THEN
        RAISE NOTICE 'SUCCESS: SHARING SYSTEM COMPLETELY REMOVED - No sharing objects remain';
    ELSE
